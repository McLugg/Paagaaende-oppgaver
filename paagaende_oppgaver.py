...
if "new_wait_for" not in st.session_state:
...
with st.expander("➕ Legg til ny oppgave", expanded=True):
    ...
    if st.session_state.new_wait:
        st.text_input("Hva venter du på?", key="new_wait_for")
    if st.button("Legg til oppgave"):
        if not st.session_state.new_title:
            ...
            # Clear inputs by deleting keys and rerun
            for key in ["new_title", "new_desc", "new_wait", "new_wait_for"]:
