...
def load_data():
    # Return empty structure if file missing or invalid
...
for idx, o in enumerate(list(oppgaver)):
    with st.expander(o["tittel"]):
        ...
        cols = st.columns(10)
        for i in range(10):
            checked = cols[i].checkbox("", value=o["status"][i], key=f"{idx}-{i}")
            o["status"][i] = checked
        prosent = sum(o["status"]) * 10
        st.progress(prosent/100)
        st.write(f"{prosent}% fullført")
        if prosent == 100:
