...
for person in PERSONER:
    ...
    # Sorter oppgaver: først venter, så nesten ferdige
...
for i in range(10):
    ...
    oppgave["status"][i] = cols[i].checkbox(label, value=oppgave["status"][i], key=f"{person}_{idx}_{i}", label_visibility="collapsed")
        ...
        st.progress(prosent, text=None)
st.markdown(f"""<div style="height: 8px; background-color: #5FAA58; width: {prosent}%; border-radius: 6px;"></div>""", unsafe_allow_html=True)
