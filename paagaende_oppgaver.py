...
for o in list(oppgaver):
    with st.expander(o["tittel"]):
        ...
        # Slider tied to unique task ID
        key = f"slider_{o['id']}"
        procent = st.slider(
            label=f"Fremgang oppgave: {o['tittel']}", 
            min_value=0, max_value=100, value=o.get("status", 0),
            step=10, key=key, label_visibility="collapsed"
        )
        o["status"] = procent
