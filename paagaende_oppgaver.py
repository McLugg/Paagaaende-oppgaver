...
# --- Hjelpefunksjon for overraskelse ---
def get_surprise_html():
    t = st.session_state.get("surprise_type", "GIF")
    if t == "GIF":
        url = st.session_state.get("surprise_gif_url", "")
        return f'<div style="text-align:center; margin:16px 0;"><img src="{url}" alt="Surprise GIF" width="200"/></div>'
    if t == "Image":
        url = st.session_state.get("surprise_img_url", "")
        return f'<div style="text-align:center; margin:16px 0;"><img src="{url}" alt="Surprise Image" width="200"/></div>'
    if t == "CSS":
        return st.session_state.get("surprise_css", "")
    return ""
...
def on_slider_change(task_id: str):
    ...
    new_val = st.session_state.get(key, task["progress"])
...
col1.metric("Oppgaver totalt", len(st.session_state.tasks))
col2.metric("Oppgaver fullført", st.session_state.completed_count)
st.markdown("---")

# --- Pågående oppgaver (før form) ---
    ...
    with st.expander(header):
        ...
        st.markdown(f"""
            <div style="background:#222;border:2px solid #5FAA58;border-radius:4px;height:24px;position:relative;">
              ...
              <div style="position:absolute;top:0;left:0;width:100%;text-align:center;line-height:24px;
                font-family:'Press Start 2P',monospace;color:#FFF;font-size:12px;">{percent}%</div>
            </div>
        """, unsafe_allow_html=True)
        if percent == 100:
...
if to_remove:
    st.session_state.tasks = [t for t in st.session_state.tasks if t.get("id") not in to_remove]
    save_tasks()
...
with st.expander("⚙️ Innstillinger", expanded=False):
    st.radio("Velg overraskelsestype", ["GIF", "Image", "CSS"], key="surprise_type")
    st.text_input("GIF URL", key="surprise_gif_url")
    st.text_input("Image URL", key="surprise_img_url")
    st.text_area("CSS-banner HTML/CSS", key="surprise_css")
    st.text_input("Marquee-symbol", key="surprise_marquee_symbol")
